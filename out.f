...
C LOCAL VARIABLES
     ...
     &        mson,mt,mvis,mxx(24),n,notuse
      INTEGER INDEX
      LOGICAL kok
      integer n_mix_prod        ! номер строки в массиве компонентов
...
C***********************************************************************
      ENTRY OUT3
      n_mix_prod=0        ! номер строки в массиве компонентов
      tra = 5.D-6
...
! для передачи данных о составе продуктов  
              n_mix_prod=n_mix_prod+1
              x__(n_mix_prod,1:Npt)=X(1:Npt)
              ProdNames(n_mix_prod)=Prod(k)
            ELSE
