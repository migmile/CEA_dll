...
!              end if
     ...
     &                     rm,((rcf(n,i,j),i=1,9),j=1,icf)
            ELSE
              READ (IOTHM) sub,nint,date,(el(j),bb(j),j=1,5),ifaz,t1,t2,
     &                     rm,eform
              IF ( nint.GT.0 ) READ (IOTHM)((rcf(n,i,j),i=1,9),j=1,nint)
            ENDIF
!            f_pos=ftell(IOTHM)
...
!!!!!! проверка температуры
 ...
 5            IF ( Tt.EQ.0. ) THEN
              ...
              IF ( rcoefs.and.hok ) THEN
                ...
                l_t(n) = 1
                IF ( ifaz.LE.0 ) THEN
                  IF ( Tt.GT.Tg(2) ) l_t(n) = 2
                  IF ( Tt.GT.Tg(3) ) l_t(n) = 3
                ENDIF
	l=l_t(n)
      if (l>0) then
            ...
            IF ( .NOT.Moles ) THEN
              DO n = 1,Nreac
                IF ( Fox(n)(:1).NE.'O'.OR.kr.NE.2 ) THEN
                  IF ( Fox(n)(:1).EQ.'O'.OR.kr.NE.1 ) 
     &                 Pecwt(n) = Pecwt(n)/Wp(kr)
                ENDIF
              ENDDO
            ENDIF
          ENDIF
